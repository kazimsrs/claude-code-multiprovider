...
#  and the environment variables this project created.
...
$vars = @(
  ...
  'ANTHROPIC_DEFAULT_HAIKU_MODEL','CLAUDE_CODE_SUBAGENT_MODEL','CLAUDE_CODE_EFFORT_LEVEL','CLAUDE_CODE_DISABLE_UNKNOWN_MODEL_WINDOW_ENFORCEMENT',
  ...
  'OPENROUTER_API_KEY','OPENAI_MODEL','GEMINI_API_KEY','GEMINI_MODEL','XAI_MODEL','MISTRAL_MODEL','LLAMA_MODEL','NEMOTRON_MODEL'
)
...
Write-Host ''
Write-Host 'Done. Claude Code itself was NOT removed. `.local\bin` stays on PATH (harmless).' -ForegroundColor Cyan
